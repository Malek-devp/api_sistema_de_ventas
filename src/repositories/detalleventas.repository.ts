...
export class DetalleVentasRepository {
    async findAll(): Promise<DetalleVentas[]> {
        try {
            ...
            throw new Error(`Error: ${error}`); // FIX: throw directo, no new Error({message:error}) que produce "[object Object]"
        }
    }

...
export class VentasRepository {
    ...
    async create(id_usuario: number): Promise<Ventas> {
        try {
    ...
    async update(id: number): Promise<AnularVentaResponse> {
        ...
        try {
            ...
            for (const articulo of resDetalle.rows) {
                ...
                WHERE id = $1;
            `;
                await client.query(queryStock, [articulo.producto_id, articulo.cantidad]);
            }

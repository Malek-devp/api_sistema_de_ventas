...
export async function getProductosDB() {
    try {
        const result = await pool.query(`SELECT p.*, c.nombre AS categoria_nombre
             FROM productos p
...
export async function postProductosDB(producto) {
    try {
...
export async function putProductosDB(id, producto) {
    try {
        const { marca, precio, stock, id_categoria } = producto;
